...
void clockReceiverFuncCtrl(void)
{
	...
	mask = 0x01;
	for (i = 0; i < 2; i++) {
		if ((CV33_36[i + 2] & 0x1F) == 0x1F) {
			// Analog Function On
			if ((CV33_36[i + 2] & 0xC0) == 0) {
				funcPortStat |= (1 << i);
			} else {
...
void funcSet(uint8_t funcNum, uint8_t stat, uint8_t direction)
{
	...
	*/
	...
	funcSetVirtual(0, CV33_36[0], funcNum, stat);
	funcSetVirtual(1, CV33_36[1], funcNum, stat);
	funcSetPort(0, CV33_36[2], funcNum, stat, direction);
	funcSetPort(1, CV33_36[3], funcNum, stat, direction);
...
void funcSetPort2(uint8_t funcPort, uint8_t count)
{
	...
	*/
	...
	if (funcPort == 0) {
		//cvAddr = CV35;
		//cvType = CV114;
		cvType = CV112_CV115[2];
	} else if (funcPort == 1) {
		//cvAddr = CV36;
		//cvType = CV115;
		cvType = CV112_CV115[3];
	} else {
		return;
	}
	...
	if ((cvType & 0x0F) == 0x01) {
		...
		if (funcVirtualStat & 0x04) {
			// Headlight Only
			illuminateValue = (CV112_CV115[0] & 0xF0) >> 4;
